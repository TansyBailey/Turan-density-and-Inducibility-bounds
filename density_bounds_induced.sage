# Version of the code in density_bounds that uses induced F-free graphs
...
# Function to generate all unlabelled F-free graphs
def induced_F_free_rgraphs(r,k,F):
    ...
    F_free = [g for g in G_structures 
              if sum([int(sum(1 for _ in g.isomorphic_substructures_iterator(f,induced=True)) > 0) 
              for f in F]) == 0]
...
def induced_main_function(r,k,F,P,problem):

    # Generate unlabelled graphs of order k:
    k_graphs_list = induced_F_free_rgraphs(r,k,F)
    ...
    for v in orders:
        ...
        graphs_order_v = induced_F_free_rgraphs(r,v,F)
        ...
        unlabelled_flags = induced_F_free_rgraphs(r,num_t_flag_verts,F)
